...
static s32 update_camera_skip_state(ViewportNode *node) {
    ...
    if (!cameraSkipState->valid) {
        ...
        for (row = 0; row < 3; row++) {
            for (col = 0; col < 3; col++) {
                cameraSkipState->prevRot[row][col] = node->modelingMatrix.m[row][col];
            }
        }
    ...
    rotationCut = camera_rotation_exceeds_threshold(cameraSkipState->prevRot, node->modelingMatrix.m);
    for (row = 0; row < 3; row++) {
        for (col = 0; col < 3; col++) {
            cameraSkipState->prevRot[row][col] = node->modelingMatrix.m[row][col];
        }
